	...
	__item[k1, p*] {
		...
		set {
			if	p.length  						; If there are still parameters remaining
				...
				|| !(base[k1] is this.class))	; or it was set but is not a map_n. (need this check to allow overwriting existing indices)
				; Then:
				base[k1] := this.class.new()	; store a sub-map (map_n) here. This sub-map will either hold the value,
